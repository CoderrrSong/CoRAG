...
def run_test(e, dataTy,modelTy, stage, methodTy,batch_size=16): 
    print(f"\n{modelTy}{dataTy}{stage}...",flush=True)

    #reranker
    if modelTy=="FT":
        RERANKER_PATH = "../reranker/bge-reranker-v2-m3-ft"
    else:
        RERANKER_PATH = "../reranker/bge-reranker-v2-m3"

    ranker, ranker_tokenizer, device = load_ranker(RERANKER_PATH)


    reranker_dir = f"reranker{methodTy}"
    ...
    generator_dir = f"generator{methodTy}"
    ...
    test_data_list = load_json_for_test(f"./data/{dataTy}/ori_data/test.json")
    #调试使用
