...
def myReward(completions, **kwargs):
    ...
    with open(f"./data_v{ver}/{dataTy}/epoch_{epoch+1}.jsonl", "a", encoding="utf-8") as f:
...
def reranker_training(device, extract_data_list, ranker, tokenizer_ranker, optimizer_ranker, epoch, dataTy, batch_size, ver):
    ...
    save_path = os.path.join(f"data_v{ver}/{dataTy}/epoch_{epoch}_reranker_results.json")
...
if __name__ == "__main__":
    ...
    print("\nRanker init")
    ranker_path = "../reranker/bge-reranker-v2-m3"
    ...
    print("\nGenerator init")
    base_model = '../generator/Meta-Llama-3.1-8B-Instruct'
    ...
    model = prepare_model_for_kbit_training(llm_model, use_gradient_checkpointing=False)
    ...
    print("\nload ")
    extract_data_list_path = f"./data_v{ver}/{dataTy}/train_labels_list.json"
    extract_data_list = load_json(extract_data_list_path)
    ...
    for epoch in tqdm(range(epochNum), desc="Epoch Progress"):
        if epoch != 0:
            extract_data_list_path = f"./data_v{ver}/{dataTy}/epoch_{epoch}.jsonl"
            extract_data_list = load_jsonlines(extract_data_list_path)
        ...
        reranker_choose_path = os.path.join(f"data_v{ver}/{dataTy}/epoch_{epoch}_reranker_results.json")
...
{x['question']}
        ...
        print(f"\n\nsave...")
        ranker.save_pretrained(f"reranker{methodTy}")
        llm_model.save_pretrained(f"./generator{methodTy}")
        tokenizer_llm.save_pretrained(f"./generator{methodTy}")
        
        # test
        ACC = run_test(e=epoch, dataTy=dataTy, modelTy=modelTy, stage=3, methodTy=methodTy)
        if ACC > best_acc:
            best_acc = ACC
            ranker.save_pretrained(f"reranker{methodTy}_best")
            llm_model.save_pretrained(f"./generator{methodTy}_best")
